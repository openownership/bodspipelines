...
#          patch('bodspipelines.infrastructure.clients.kinesis_client.create_client') as mock_kn):
        ...
        assert item['LEI'] == '00X5RQKJQQJFFX0WPA53'
        assert item['Entity']['LegalName'] == "The Greater Morristown Young Men's Christian Association, Inc."
        assert item['Entity']['OtherEntityNames'] == [{'type': 'PREVIOUS_LEGAL_NAME', 'OtherEntityName': 'The Greater Morristown YMCA Inc'}]
        assert item['Entity']['LegalAddress'] == {'FirstAddressLine': '79 Horsehill Road', 'City': 'Cedar Knolls', 'Region': 'US-NJ', 
                                                  'Country': 'US', 'PostalCode': '07927-2003'}
        assert item['Entity']['HeadquartersAddress'] == {'FirstAddressLine': '79 Horsehill Road', 'City': 'Cedar Knolls', 'Region': 'US-NJ', 
                                                         'Country': 'US', 'PostalCode': '07927-2003'}
        assert item['Entity']['RegistrationAuthority'] == {'RegistrationAuthorityID': 'RA999999'}
        assert item['Entity']['LegalJurisdiction'] == 'US-NJ'
        assert item['Entity']['EntityCategory'] == 'GENERAL'
        assert item['Entity']['LegalForm'] == {'EntityLegalFormCode': 'XSNP'}
        assert item['Entity']['EntityStatus'] == 'ACTIVE'
        assert item['Registration']['InitialRegistrationDate'] == '2012-11-22T10:55:00.000Z'
        assert item['Registration']['LastUpdateDate'] == '2020-07-24T19:29:00.000Z'
        assert item['Registration']['RegistrationStatus'] == 'LAPSED'
        assert item['Registration']['NextRenewalDate'] == '2013-11-22T10:38:00.000Z'
        assert item['Registration']['ManagingLOU'] == 'EVK05KS7XY1DEII3R011'
        assert item['Registration']['ValidationSources'] == 'ENTITY_SUPPLIED_ONLY'
        assert item['Registration']['ValidationAuthority'] == {'ValidationAuthorityID': 'RA999999'}
...
def test_lei_transform_stage(lei_list, json_data_file):
    ...
    #      patch('bodspipelines.infrastructure.clients.kinesis_client.create_client') as mock_kn):
        ...
        async def result():
            for lei in lei_list:
                yield (True, {'create': {'_id': generate_statement_id(lei, 'entityStatement')}})
        mock_sb.return_value = result()
        put_future = asyncio.Future()
        put_future.set_result(None)
        mock_kno.return_value.put.return_value = put_future
        ...
        print(item)
        #item = json.loads(mock_kn.return_value.put_records.call_args.kwargs['Records'][0]['Data'])
        assert item['statementID'] == '28b2095f-e68e-9cc0-7bec-bad913797e19'
        assert item['statementType'] == 'entityStatement'
        assert item['statementDate'] == '2020-07-24'
        assert item['entityType'] == 'registeredEntity'
        assert item['name'] == "The Greater Morristown Young Men's Christian Association, Inc."
        assert item['incorporatedInJurisdiction'] == {'name': 'US-NJ', 'code': 'US-NJ'}
        assert {'id': '00X5RQKJQQJFFX0WPA53', 'scheme': 'XI-LEI', 'schemeName': 'Global Legal Entity Identifier Index'} in item['identifiers']
        assert {'schemeName': 'RA999999'} in item['identifiers']
        assert {'type': 'registered', 'address': '79 Horsehill Road, Cedar Knolls', 'country': 'US', 'postCode': '07927-2003'} in item['addresses']
        assert {'type': 'business', 'address': '79 Horsehill Road, Cedar Knolls', 'country': 'US', 'postCode': '07927-2003'} in item['addresses']
